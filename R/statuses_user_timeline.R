...
#'      Example Values: 12345
#' @param count
#'      Specifies the number of tweets to try and retrieve, up to a maximum of 200 per distinct request. The
#'      value of count is best thought of as a limit to the number of tweets to return because suspended or
#'      deleted content is removed after the count has been applied. We include retweets in the count, even
#'      if include_rts is not supplied. It is recommended you always send include_rts=1 when using this API
#'      method.
#' @param max_id
...
#'      both the maximal length of the timeline and the slice selected by the count parameter). Note:
#'      If youÅfre using the trim_user parameter in conjunction with include_rts, the retweets will still
#'      contain a full user object.
#'      Example Values: false
#' @export
twtr_statuses_user_timeline <- function(user_id = NULL, screen_name = NULL, since_id = NULL, count = NULL, max_id = NULL, trim_user = NULL, 
    exclude_replies = NULL, contributor_details = NULL, include_rts = NULL, ...) {
    twtr_api("GET", "https://api.twitter.com/1.1/statuses/user_timeline.json", query = list(user_id = user_id, screen_name = screen_name, since_id = since_id, 
        count = count, max_id = max_id, trim_user = trim_user, exclude_replies = exclude_replies, contributor_details = contributor_details, 
